...
@WebServlet(urlPatterns = {"/Admin_Logout"})
public class Admin_Logout extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String admin_type = request.getParameter("type");
            HttpSession session = request.getSession();
            if (admin_type != null) {
                System.out.println(admin_type);
                if(admin_type.equals("shopkeeper")) {
                    session.setAttribute("type", "shopkeeper");
                    session.removeAttribute("skuser");
                } else if (admin_type.equals("servicer")) {
